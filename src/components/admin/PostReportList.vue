    ...
    <div class="container">
      ...
      <div class="collection">
        ...
        <router-link
          ...
          :key="one.id"
        >
          <span class="col s2 center-align"> {{ one.postId }} </span>
...
import { reportPosts } from '@/api/admin';
...
export default {
  ...
  methods: {
    searching() {
      ...
      if (keyword !== '') {
        this.$router.push(
          `/admin/reports/posts?page=1&${qstr.stringify(this.search)}`,
        );
      }
    },
    ...
    beforeLoadPage() {
      ...
      if (this.query.page === undefined) {
        this.$router.push({ path: '/admin/reports/posts', query: { page: 1 } });
      } else {
        this.loadPage();
      }
    },
    ...
    async loadPage() {
      const query =
        this.$route.query.page !== undefined
          ...
          : 'page=1';
      const res = await reportPosts(query);
    ...
    fullPath(val) {
      ...
      target.page = val;
      return { path: '/admin/reports/posts', query: target };
    },
